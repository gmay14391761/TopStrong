...
namespace Easy4net.DBUtility
{
    public class AdoHelper
    {
        ...
        //获取数据库连接字符串
        public static string ConnectionString = ConfigurationManager.ConnectionStrings["ConnectionSQL"].ToString();//ConfigurationManager.AppSettings["ConnectionSQLST"].ToString();
        ...
        /// <param name="commandParameters">执行命令所需的参数数组</param>
        /// <returns>返回SqlDataReader对象</returns>
        public static IDataReader ExecuteReader(bool closeConnection, IDbTransaction transaction, IDbConnection connection, CommandType cmdType, string cmdText, params IDbDataParameter[] commandParameters)
        {
            IDbCommand cmd = DbFactory.CreateDbCommand();
            IDbConnection conn = connection;
        ...
        private static bool GetNum(string str, string seachstr)
        {
            int len = seachstr.Length;
            string r = str.Replace(seachstr, "");
            int num = (str.Length - r.Length) / len;
            if (num > 1)
                return true;
            else
                return false;
